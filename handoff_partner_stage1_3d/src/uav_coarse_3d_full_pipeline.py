...
def astar_3d(
    ...
    if best_goal_state is None:
        # Safe fallback: climb above all obstacles and connect directly in 3D.
        cruise_z = max(max_z - 2.0, start[2], goal[2])
        fallback = [
            start,
...
def order_baseline(path_scenario: PathScenario3D) -> List[int]:
...
def order_hierarchical(path_scenario: PathScenario3D) -> List[int]:
    points = path_scenario.selected_points
    owners = path_scenario.owners
    cluster_ids = path_scenario.cluster_ids
    sector_ids = path_scenario.sector_ids
    building_to_indices: Dict[str, List[int]] = {}
    for idx, owner in enumerate(owners):
        building_to_indices.setdefault(owner, []).append(idx)
    building_centroids = {
        owner: np.mean(points[indices, :2], axis=0)
        for owner, indices in building_to_indices.items()
    }
    building_keys = sorted(
        building_to_indices,
        key=lambda name: (name == "unassigned", name),
    )
    building_order = nearest_centroid_order(building_keys, building_centroids, path_scenario.start)

    current_point = np.asarray(path_scenario.start, dtype=np.float64)
    final_order: List[int] = []
    for owner in building_order:
        owner_indices = building_to_indices[owner]
        sector_to_indices: Dict[int, List[int]] = {}
        for idx in owner_indices:
            sector_to_indices.setdefault(int(sector_ids[idx]), []).append(idx)
        sector_centroids = {
            str(sector_id): np.mean(points[indices, :2], axis=0)
            for sector_id, indices in sector_to_indices.items()
        }
        sector_keys = [str(sector_id) for sector_id in sector_to_indices]
        sector_order = nearest_centroid_order(sector_keys, sector_centroids, current_point)
        for sector_key in sector_order:
            sector_indices = sector_to_indices[int(sector_key)]
            cluster_to_indices: Dict[int, List[int]] = {}
            for idx in sector_indices:
                cluster_to_indices.setdefault(int(cluster_ids[idx]), []).append(idx)
            cluster_centroids = {
                str(cluster_id): np.mean(points[indices, :2], axis=0)
                for cluster_id, indices in cluster_to_indices.items()
            }
            cluster_keys = [str(cluster_id) for cluster_id in cluster_to_indices]
            cluster_order = nearest_centroid_order(cluster_keys, cluster_centroids, current_point)
            for cluster_key in cluster_order:
                cluster_indices = cluster_to_indices[int(cluster_key)]
                ordered_cluster = order_points_from_anchor(cluster_indices, points, current_point)
                final_order.extend(ordered_cluster)
                current_point = points[ordered_cluster[-1]]
    return two_opt_refine(path_scenario, final_order, max_passes=3)


def assemble_path_3d(path_scenario: PathScenario3D, order: Sequence[int]) -> List[Point3]:
    node_order = [0] + [index + 1 for index in order] + [0]
    ...
    if len(path_3d) >= 4:
        takeoff = spiral_transition_3d(path_3d[0], path_3d[1], turns=0.55, radius=6.5, samples=16)
        landing = spiral_transition_3d(path_3d[-2], path_3d[-1], turns=0.55, radius=6.5, samples=16)
        path_3d = takeoff[:-1] + path_3d[1:-2] + landing
    return path_3d
...
def write_pipeline_outputs(
    ...
    with (output_dir / "coarse_3d_full_pipeline_results.md").open("w", encoding="utf-8") as stream:
        ...
        stream.write("主线：候选视点生成（法向+分区/聚类） -> 视点选取（Maskable PPO best-of-N，含 shots/standoff） -> 路径规划（building -> sector/cluster -> 2-opt -> 安全直连优先，否则 3D A* -> 平滑）。\n\n")
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--selection-seed-sweep", type=str, default="7,23,29")
...
def main() -> None:
    ...
    scenario = compress_candidates(base_scenario, args.ppo_candidates)
    ...
    ppo_selection = ppo_best
    path_scenario = build_path_scenario(scenario, ppo_selection, args.scene_json)
    baseline_order = order_baseline(path_scenario)
    baseline_path = evaluate_path_method(path_scenario, baseline_order, "Maskable PPO best-of-N path baseline")
    hierarchical_order = order_hierarchical(path_scenario)
    hierarchical_path = evaluate_path_method(path_scenario, hierarchical_order, "Maskable PPO best-of-N + hierarchical path agent")

    output_dir = args.output_dir
    output_dir.mkdir(parents=True, exist_ok=True)
    selection_model.save(output_dir / "selection_maskable_ppo")
    write_waypoints_csv(baseline_path, output_dir / "baseline_path_waypoints.csv")
    write_waypoints_csv(hierarchical_path, output_dir / "hierarchical_path_waypoints.csv")
    write_selected_capture_csv(path_scenario, baseline_order, output_dir / "baseline_selected_captures.csv")
    write_selected_capture_csv(path_scenario, hierarchical_order, output_dir / "hierarchical_selected_captures.csv")
    plot_path_3d(scenario, path_scenario, baseline_path, output_dir / "baseline_path_3d.png")
    plot_path_3d(scenario, path_scenario, hierarchical_path, output_dir / "hierarchical_path_3d.png")
    write_pipeline_outputs(
        [greedy, ilp, ppo_selection],
        seed_runs,
        [baseline_path, hierarchical_path],
        output_dir,
    )
    ...
    print(
        f"Path baseline: raw={baseline_path.path_length_m:.1f}, smooth={baseline_path.smooth_path_length_m:.1f}"
    )
    print(
        f"Hierarchical path agent: raw={hierarchical_path.path_length_m:.1f}, smooth={hierarchical_path.smooth_path_length_m:.1f}"
    )
