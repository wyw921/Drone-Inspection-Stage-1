    ...
    if np.any(remaining_cover > 0):
        ...
        if len(supplemental_local):
            ...
            if len(supplemental_local):
                ...
                rescue_scores = (
                    1.6 * supplemental_coverage[:, missing_indices].sum(axis=1).astype(np.float32)
                    + 2.0 * supplemental_quality[:, missing_indices].sum(axis=1).astype(np.float32)
                )
                ...
                supplemental_meta = [
                    {"cluster_id": -2.0, "region_code": 2.0, "importance": 0.78}
